...
def main():
    ...
    rows = [
        ...
        {"architecture": "legacy validator workflow", "execution": "MEASURED", "same_chapter": True, "model_tokens": "not measured", "model_latency_ms": "not measured", "deterministic_latency_ms": {"quick_real_s": real_time(legacy_quick_time), "strict_real_s": real_time(legacy_strict_time)}, "quick_status": "PASS" if legacy_quick.get("passed") else "FAIL", "strict_status": "PASS" if legacy_strict.get("passed") else "FAIL", "beats_status": "WARN: beat labels heuristic", "finding_count": counts(legacy_quick)["findings"] + counts(legacy_strict)["findings"], "quick_findings": counts(legacy_quick), "strict_findings": counts(legacy_strict), "evidence": ["legacy-quick.json", "legacy-strict.json", "legacy-quick.time", "legacy-strict.time"]},
